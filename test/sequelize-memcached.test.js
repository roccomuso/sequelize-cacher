...
/* Sequelize options */
var opts = {};
opts.database = process.env.DB_NAME || 'sequelize_memcached_test';
...
describe('Sequelize-Memcached-Cache', function() {
  var memcached;
  var db;
  var Entity;
  var Entity2;
  ...
  before(function(done) {
    ...
    memcached.on('failure', function( details ){ onErr( "Server " + details.server + "went down due to: " + details.messages.join( '' ) ); });

    db = new Sequelize(opts.database, opts.user, opts.password, opts);
    cacher = initCache(db, memcached);
